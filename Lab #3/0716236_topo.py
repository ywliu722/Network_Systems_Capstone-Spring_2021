...
def config(routers, h2, h3, h4, DHCPServer):
    ...
    #SNAT for part2
    routers['r1'].cmd('iptables -t nat -A POSTROUTING -s 192.168.1.0/26 -j SNAT --to 140.113.0.30')
    routers['r1'].cmd('iptables -t nat -A POSTROUTING -s 192.168.1.64/26 -j SNAT --to 140.113.0.40')
    #DNAT for part2
    routers['r1'].cmd('iptables -t nat -A PREROUTING -p tcp -d 140.113.0.40 --dport 80 -j DNAT --to 192.168.1.66:8000')
