...
for i=1:length(NumSta)
    for j=1:NumIter
        ...
        backoff=randi([1,CWmin],1,nn)               % Initial backoff counter
                ...
                if (min(backoff(uid)) == 0)
                    ...
                    if(length(winner) == 1)
                        ...
                        for k=1:length(winner)
                            CW(winner(k))=min(CW(winner(k))*2,CWmax);
                        end
                    end
                    ...
                    for k=1:length(winner)
                        backoff(winner(k)) = randi([1,CW(winner(k))],1)
                    end                          
...
for u=1:length(InterPktTime)
    if(floor(InterPktTime(u)) == 0)
        ...
        p=rand();
        InterPktTime(u)=-log(1-p)/(lambda(u)*SlotTime);   % TODO: update the interarrival time here
