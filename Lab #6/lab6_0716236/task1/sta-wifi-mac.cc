      ...
      if (goodBeacon && m_state == ASSOCIATED)
        {
          ...
          NS_ASSERT (removed);
          double m_snr = snrTag.Get();
          double m_snr_log = 10*log10(m_snr);
          if(m_snr_log < m_snrThreshold){
            SendDisassociationMsg();
            SetState(UNASSOCIATED);
            m_activeProbing = true;
            TryToEnsureAssociated();
            return;
          }
...
StaWifiMac::UpdateCandidateApList (ApInfo newApInfo)
{
  ...
  double m_snr_log = 10*log10(newApInfo.m_snr);
  if(m_snr_log < m_snrThreshold){
    return;
  }
