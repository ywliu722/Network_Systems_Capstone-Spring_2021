...
void Movement (Ptr<Node> node, std::vector<Vector> &boundary, std::vector<MOVE> &moves, int index, int iteration, double stepsTime);
...
int main (int argc, char *argv[])
{
  ...
  for(int i=0;i<100;i++){
    testing = GetSingleMovement(fp);
    if(testing.index==0 && i!=0){
      break;
    }
    moves.push_back(testing);
  }
  Simulator::Schedule (Seconds (stepsTime), &Movement, wirelessNodes.Get (0), boundary, moves, 0, 1, stepsTime);
...
void Movement (Ptr<Node> node, std::vector<Vector> &boundary, std::vector<MOVE> &moves, int index, int iteration, double stepsTime)
{
  ...
  if(iteration%8==0){
    iteration=0;
    pos.x=pos.x+(moves[index].speed * cos(moves[index].rad)) / 8;
    pos.y=pos.y+(moves[index].speed * sin(moves[index].rad)) / 8;
    //std::cout<<"Current Position: ("<<pos.x<<", "<<pos.y<<", "<<pos.z<<")"<<std::endl;
    index++;
    while(1){
      double new_x=pos.x+moves[index].speed * cos(moves[index].rad);
      double new_y=pos.y+moves[index].speed * sin(moves[index].rad);
      if(new_x<boundary[1].x || new_x>boundary[0].x || new_y<boundary[1].y || new_y>boundary[0].y){
        PrintIllegalMsg(index);
      }
      else{
        break;
      }
    ...
    //std::cout<<"NEXT MOVE: "<<moves[index].index<<" "<<moves[index].speed * cos(moves[index].rad)<<" "<<moves[index].speed * sin(moves[index].rad)<<std::endl;
    //CheckLinkState(node); 
  }
  else{
    pos.x=pos.x+(moves[index].speed * cos(moves[index].rad)) / 8;
  ...
  iteration++;
  //CheckLinkState(node); 
  Simulator::Schedule (Seconds (stepsTime), &Movement, node, boundary, moves, index, iteration, stepsTime);
}
